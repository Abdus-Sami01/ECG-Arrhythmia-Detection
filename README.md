...
- [x] Phase 5 — Pan-Tompkins R-peak front end (batch + streaming), DS2 evaluation (F1 0.9916).
...
## R-peak front end — Pan-Tompkins detector

`pan_tompkins.py` closes the gap from beat classification to a real streaming pipeline.

**Offline batch detector** (`detect_beats`): zero-phase Butterworth bandpass (5–15 Hz, 1st order)
→ 5-point derivative → squaring → 150 ms moving-window integration → adaptive dual-threshold peak
detection with 200 ms refractory period.

**Streaming class** (`PanTompkinsDetector`): identical signal chain with causal IIR filters and a
circular integration buffer, processing one sample per call.  This maps directly to a
microcontroller ISR or DMA callback; the class holds all state internally with no heap allocation
after construction.

**DS2 evaluation** (22 test records, ±150 ms tolerance, ground-truth WFDB annotations):
...
| PPV | 0.9855 |
| F1 | 0.9916 |
| TP / FP / FN | 49 599 / 728 / 113 |

Literature (Pan & Tompkins 1985, full MIT-BIH): sensitivity 99.3%, PPV 99.3%.  These DS2-only
numbers are consistent.  The 2 s initialisation window is the streaming detector's only latency
overhead — thereafter every detected R-peak is available immediately for beat segmentation.
...
python pan_tompkins.py         # R-peak detector eval -> results/pan_tompkins_eval.json
```
...
- **Pan-Tompkins uses ground-truth annotations for the classification evaluation** — the detector
  and the classifier are evaluated independently here; end-to-end error propagation (detector
  misses feeding wrong windows to the classifier) is not measured.
...
pan_tompkins.py      Pan-Tompkins R-peak detector (batch + streaming), DS2 evaluation
